...
def clean_data(uploaded_file):
    ...
    df['Credit_Score'] = df['Credit_Score'].fillna(10) # 預設補 BBB-
        ...
        if st.sidebar.button("🚀 開始計算最佳組合"):
            ...
            if res.success:
                ...
                with c2:
                    ...
                    # 加入 key 避免渲染錯誤
                    st.plotly_chart(fig, use_container_width=True, key="pie_chart") # <--- 加了這個
                    ...
                    # 加入 key 避免渲染錯誤
                    st.plotly_chart(fig2, use_container_width=True, key="scatter_chart") # <--- 加了這個
